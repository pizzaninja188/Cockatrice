...
TEST_F(RuledE2ESmokeTest, HeatedArgumentKeepsGraveyardChoicePrivateAndMovesTheExactCard)
{
    const auto started = startServers();
    ASSERT_TRUE(started) << started.message();
    if (std::string(started.message()).rfind("SKIP:", 0) == 0) {
    ...
    OpeningDriver p1(true, QStringLiteral("heatedp1"), &transcript);
    OpeningDriver p2(false, QStringLiteral("heatedp2"), &transcript);
    ASSERT_TRUE(p1.loginAndJoinRoom());
    ASSERT_TRUE(p2.loginAndJoinRoom());
    ASSERT_TRUE(p1.createRuledGame());
    ASSERT_TRUE(p2.joinRuledGame(p1.gameId));
    ...
    p2.sendReady();
    ASSERT_TRUE(p1.pumpUntil([&] { return p1.gameStarted && p1.stateVersion > 0; }, 20000,
                             "Heated Argument start p1"));
    ASSERT_TRUE(p2.pumpUntil([&] { return p2.gameStarted && p2.stateVersion > 0; }, 20000,
                             "Heated Argument start p2"));
    ASSERT_TRUE(p1.publishMain1Stops());
    ASSERT_TRUE(p2.publishMain1Stops());
    QElapsedTimer opening;
    opening.start();
    ...
    auto send = [&](OpeningDriver &sender, const ruled::v1::RuledCommand &command) {
        const auto before1 = p1.stateVersion;
        const auto before2 = p2.stateVersion;
        sender.sendRuled(command, QStringLiteral("Heated Argument scenario"));
        QElapsedTimer wait;
        wait.start();
        while ((p1.stateVersion <= before1 || p2.stateVersion <= before2) && wait.elapsed() < 10000) {
            p1.pump(25);
    ...
    move.mutable_dev_command()->set_target_player_id(p1.myId);
    move.mutable_dev_command()->mutable_move_card()->set_card_name("Lightning Bolt");
    move.mutable_dev_command()->mutable_move_card()->set_zone(ruled::v1::DEV_ZONE_GRAVEYARD);
    ASSERT_TRUE(send(p1, move));
    ASSERT_EQ(p1.graveyardOwnerByEngineOid.size(), 1u);
    const quint32 graveyardOid = p1.graveyardOwnerByEngineOid.begin()->first;
    ASSERT_EQ(p2.graveyardOwnerByEngineOid[graveyardOid], p1.myId);
    ASSERT_TRUE(p1.serverCardByEngineOid.count(graveyardOid));
    const int physicalGraveyardCard = p1.serverCardByEngineOid[graveyardOid];
    ASSERT_EQ(p2.serverCardByEngineOid[graveyardOid], physicalGraveyardCard);

    ASSERT_TRUE(put(p1.myId, "Heated Argument", ruled::v1::DEV_ZONE_HAND));
    ruled::v1::RuledCommand mana;
    mana.mutable_dev_command()->set_target_player_id(p1.myId);
    mana.mutable_dev_command()->mutable_add_mana()->set_r(1);
    mana.mutable_dev_command()->mutable_add_mana()->set_c(4);
    ASSERT_TRUE(send(p1, mana));
    const auto *action = p1.handAction(ruled::v1::HAND_ACTION_CAST_SPELL, QStringLiteral("Heated Argument"));
    ASSERT_NE(action, nullptr);
    ...
    EXPECT_EQ(choice.choice_kind(), ruled::v1::CHOICE_KIND_GRAVEYARD_CARDS);
    ASSERT_EQ(choice.candidate_object_ids_size(), 1);
    EXPECT_EQ(choice.candidate_object_ids(0), graveyardOid);
    ASSERT_EQ(choice.candidate_source_zones_size(), 1);
    EXPECT_EQ(choice.candidate_source_zones(0), ruled::v1::CHOICE_CANDIDATE_SOURCE_ZONE_GRAVEYARD);
    EXPECT_EQ(choice.candidate_server_card_ids_size(), 1);
    // Image pickers use choice-local synthetic IDs; the engine OID is what preserves the
    // binding to the real graveyard card when the payment commits.
    ...
    ruled::v1::RuledCommand pay;
    pay.mutable_submit_resolution_choice()->add_chosen_object_ids(graveyardOid);
    p1.pendingChoice.reset();
    ASSERT_TRUE(send(p1, pay));
    for (auto *client : {&p1, &p2}) {
        EXPECT_EQ(client->lifeByPlayer[p2.myId], 18);
        const auto moved = std::find_if(client->physicalMoveEvents.begin(), client->physicalMoveEvents.end(),
                                        [&](const auto &event) {
                                            return event.start_zone() == ZoneNames::GRAVE &&
                                                   event.target_zone() == ZoneNames::EXILE &&
                                                   event.card_id() == physicalGraveyardCard;
                                        });
        ASSERT_NE(moved, client->physicalMoveEvents.end());
        EXPECT_EQ(moved->new_card_id(), physicalGraveyardCard);
    }
}
