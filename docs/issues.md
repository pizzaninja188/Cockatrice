...
- [ ] #36 [bug] A suspending effect silently drops the rest of its spell's effect list
  - Details: `resolve_top_of_stack` (`tricerules-core/src/engine/resolution/mod.rs`) returns early on `EffectOutcome::Suspended`, and the `finish_*` paths in `engine/custom_resolution.rs` (`finish_discard_chosen`, `finish_library_search`, `finish_sacrifice_chosen`, …) complete the parked effect and end the resolution — they never re-enter the effect list. So for `spell_effect: [A, B]` where A suspends for a player choice, **B never runs**, with no error and no log line. Worse, it is order-of-magnitude inconsistent for the same card: `DiscardCards(random: false)` only parks when there is a real choice to make, so a card can lose its second effect against a two-card hand and keep it against a one-card hand. Currently inert because every implemented card either suspends in its last effect or not at all — Thoughtseize's RON deliberately declares `LoseLife` *before* the suspending `DiscardCards` to dodge it (see the comment there and at the `Suspended` early return). Fix: park the index of the suspended effect alongside the choice in `PendingResolution`, and have the resume path continue the remaining effects instead of finishing the resolution; then restore Thoughtseize to Oracle order (discard, then lose 2 life) as the regression test. Add a scenario test with a two-effect spell whose first effect parks.
  - Priority: Medium
