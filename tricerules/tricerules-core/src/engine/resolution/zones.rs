...
pub(super) fn exile_target(
    ...
    if let Some(&tid) = targets.first() {
        ...
        move_object_to_zone(&mut engine.state, tid, Zone::Exile)?;
...
pub(super) fn return_target_to_hand(
    ...
    if let Some(&tid) = targets.first() {
        ...
        move_object_to_zone(&mut engine.state, tid, Zone::Hand)?;
        events.push(ev_log(format!(
            ...
            if random {
                ...
                if discard_count == 0 {
                    ...
                    for oid in &to_discard {
                        ...
                        move_object_to_zone(&mut engine.state, *oid, Zone::Graveyard)?;
                        events.push(permanent_moved_event(
                            &engine.state,
                            *oid,
                            pid,
...
pub(super) fn return_from_graveyard(
    ...
    if let Some(&tid) = targets.first() {
        ...
        if !is_legal {
            ...
            move_object_to_zone(&mut engine.state, tid, dest_zone)?;
