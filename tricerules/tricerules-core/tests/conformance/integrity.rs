...
fn try_drain_stack(e: &mut GameEngine) -> bool {
    for _ in 0..40 {
...
pub(super) fn assert_zone_integrity(e: &GameEngine, expected_objects: usize, ctx: &str) {
    ...
    // A resolving spell is held by its continuation while a choice is parked.
    let parked = e
        .state
        .pending_resolution
        .as_ref()
        .and_then(|p| p.continuation.stack())
        .map(|s| &s.item)
        .filter(|item| {
            e.state
                .objects
    ...
    for item in e.state.stack.iter().chain(parked) {
        assert!(
...
fn exercise_face_in_fresh_game(card_id: &str, face_index: u32) -> bool {
    ...
    if is_land {
        ...
        let accepted = e
            .apply_command(0, &play_land_face(slot, face_index))
            .is_ok();
        assert_zone_integrity(&e, baseline, &format!("{card_id} face {face_index} (land)"));
        return accepted && super::settled(&e);
    }
    ...
    for targets in candidates {
        ...
        if e.apply_command(0, &cast_spell_face(slot, targets, face_index))
            .is_ok()
        {
            completed = try_drain_stack(&mut e);
            break;
        }
    }
...
pub(super) fn observed_completed_cases() -> std::collections::BTreeSet<String> {
    ...
    for card_id in card_ids {
        ...
        for face_index in 1..def.face_count() as u32 {
            if exercise_face_in_fresh_game(card_id, face_index) {
                completed.insert(format!(
                    "{card_id}\t{face_index}\t{}",
                    super::play_kind(card_id, face_index as usize)
                ));
            }
        }
        ...
        if front.is_land {
            ...
            if e.apply_command(0, &play_land(slot)).is_ok() && super::settled(&e) {
        ...
        if cast_ok && try_drain_stack(&mut e) {
            completed.insert(format!("{card_id}\t0\t{}", super::play_kind(card_id, 0)));
        }
            ...
            for ai in 0..front.activated_abilities.len() as u32 {
                ...
                for targets in candidates {
                    ...
                    if ae
                        ...
                        .is_ok()
                    {
                        if try_drain_stack(&mut ae) {
                            completed.insert(format!("{card_id}\t0\tability:{ai}"));
                        }
                        break;
                    }
                }
