...
5. **This project is pre-release. Optimize for the long term.** Large changes are welcome when they leave the system simpler, more coherent, or easier to maintain. Choose the simplest implementation that fully meets the current requirements; avoid speculative abstractions, configuration, and indirection. Deliver large changes as working end-to-end increments, with each increment building on a product that still works. Do not preserve compatibility between unreleased ruled implementations, add transitional layers, or accept a stopgap merely to minimize the diff. Remove obsolete ruled paths when replacing them. This does **not** relax the requirements to preserve existing freeform behavior, keep every proto consumer aligned, or minimize the fork delta in upstream-owned files.
...
8. **Give manual test steps in the final summary when applicable.** For behavior that benefits from hands-on verification—especially UI, interaction, networking, or end-to-end ruled flows—include concise, numbered steps and the expected result. Clearly distinguish steps you performed from steps you recommend the user run.
9. **Use red/green TDD for testable behavior changes.** For bug fixes, improvements, and additions, first add a focused test that fails for the expected reason, then implement the smallest coherent change that makes it pass. Refactors should preserve green tests; add characterization coverage first when existing behavior is insufficiently protected. Do not manufacture meaningless tests for documentation, formatting, generated output, or other changes without a useful executable contract.
...
3. **Custom Rust (`tricerules-core/src/custom/` `CardEffect`)** — the escape hatch for a *resolution algorithm* data can't express. One impl per file, selected by a card's `custom_effect: "<card_id>"` RON field (mutually exclusive with `spell_effect`, enforced at registry load). Registration mirrors the RON drop-in: create `src/custom/<card_id>.rs` (any subdirectory; `support/` is skipped) exporting `pub(crate) static EFFECT: &dyn CardEffect = &YourType;`, and `build.rs` picks it up. **The card definition id, RON `custom_effect`, and file stem must all be identical**; `build.rs` prefixes the private Rust module name, so every registry-valid id—including one beginning with a digit or matching a Rust keyword—works without editing a shared Rust file. Effects are **1:1 with card ids**: duplicate file stems fail the build, multiple cards claiming one key fail the scenario suite, and two cards wanting the same algorithm is the signal to widen a primitive rather than share an impl. Resolution is **resumable**: `begin`/`resume` drive a capability-narrowed `ResolutionCtx` and either finish or return a `ResolutionInterrupt` (a player choice); the engine parks it in `GameState::pending_resolution`, emits the generic `RuledEvent.resolution_choice_required`, and resumes on the `SubmitResolutionChoice` command. Determinism holds because every choice is a logged command. Implemented: **Brainstorm**, **Gifts Ungiven**.
